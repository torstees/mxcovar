...
def parse_varid(varid):
    words = varid.split("_")
    if len(words) != 5:
        sys.stderr.write(f"There was a problem with the format of varID in {filename} - {varid}")
        sys.exit(1)
...
class Gene(object):
    ...
    def extract_weights(self, db):
        self.data = {}          # pos => Locus
        for (rsid, varid, ref, eff, weight) in db.execute(f"SELECT rsid, varID, ref_allele, eff_allele, weight FROM weights WHERE gene='{self.name}'"):
            (chr, pos, a1, a2) = parse_varid(varid)
